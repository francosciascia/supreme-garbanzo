...
import com.example.demo.dto.UsuarioDTO;import com.example.demo.dto.EmpleadoDTO;import com.example.demo.models.Persona;import com.example.demo.repository.PersonaRepository;import org.springframework.security.crypto.password.PasswordEncoder;import org.springframework.stereotype.Service;import org.springframework.transaction.annotation.Transactional;import java.util.List;
@Service
public class UsuarioService{
    private final PersonaRepository repository;private final PasswordEncoder encoder;private final EmpleadoService empleados;public UsuarioService(PersonaRepository repository,PasswordEncoder encoder,EmpleadoService empleados){this.repository=repository;this.encoder=encoder;this.empleados=empleados;}
    ...
    @Transactional public UsuarioDTO guardarCompleto(Long id,UsuarioDTO value,String password,EmpleadoDTO ficha,Long operadorId){UsuarioDTO saved=guardar(id,value,password);empleados.guardar(new EmpleadoDTO(saved.id(),saved.nombre(),saved.apellido(),saved.email(),saved.rol(),saved.activo(),ficha.legajo(),ficha.puesto(),ficha.fechaContratacion(),ficha.sueldo(),ficha.telefonoEmergencia(),ficha.observaciones(),ficha.permisos()),operadorId);return saved;}
