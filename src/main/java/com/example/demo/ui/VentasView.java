...
public class VentasView extends VerticalLayout {
    ...
    public VentasView(VentaService service, ProductoService productoService, ClienteService clienteService,
        ...
        grid.setSizeFull();
        add(ViewSupport.header("Gestión de ventas", add), search, grid, pagination);
        expand(grid);
        refresh();
    }
    ...
    private void openForm() {
        ...
        TextField barcode = new TextField("Escanear código de barras");
        barcode.setPlaceholder("Escaneá y Enter · cobrá con Ctrl+Enter");
        ...
        Runnable updateTotal = () -> {
            BigDecimal subtotal = rows.stream().map(SaleItemRow::subtotal)
                    .reduce(BigDecimal.ZERO, BigDecimal::add)
                    .subtract(discount.getValue() == null ? BigDecimal.ZERO : discount.getValue())
                    .max(BigDecimal.ZERO);
            total.setText("Total: " + CURRENCY.format(subtotal));
        ...
        Runnable focusBarcode = () -> getUI().ifPresent(ui -> ui.getPage().executeJs(
                "setTimeout(()=>$0.focus(),50)", barcode.getElement()));

        Button addItem = new Button("Agregar producto", VaadinIcon.PLUS.create());
        addItem.addClickListener(event -> addItemRow(itemContainer, rows, catalog, updateTotal));
        barcode.addKeyPressListener(Key.ENTER, event -> productoService.buscarPorCodigo(barcode.getValue()).ifPresentOrElse(product -> {
            ...
            if (existing != null) {
                existing.quantity.setValue(existing.quantity.getValue() == null
                        ? BigDecimal.ONE : existing.quantity.getValue().add(BigDecimal.ONE));
            } else {
                addItemRow(itemContainer, rows, catalog, updateTotal).product.setValue(product);
            }
            barcode.clear();
            updateTotal.run();
            focusBarcode.run();
        ...
        dialog.add(new VerticalLayout(client, barcode, addItem, itemContainer,
                new HorizontalLayout(payment, received, discount), total, change));
        addItemRow(itemContainer, rows, catalog, updateTotal);
        ...
        Button save = new Button("Crear venta", event -> {
            ...
            if (rows.isEmpty() || rows.stream().anyMatch(row -> row.product.getValue() == null || row.quantity.isEmpty())) {
                ViewSupport.error(new IllegalArgumentException("Seleccioná un producto y una cantidad en cada ítem"));
                return;
            }
            try {
                ...
                var sale = service.crear(new VentaCreateDTO(client.getValue() == null ? null : client.getValue().id(), items,
                        ...
                        caja.map(com.example.demo.dto.CajaDTO::id).orElse(null), UserSession.getUser().id()));
                refresh();
                ViewSupport.success("Venta creada");
    ...
    private SaleItemRow addItemRow(VerticalLayout container, List<SaleItemRow> rows, List<ProductoDTO> catalog,
                                   Runnable updateTotal) {
        ComboBox<ProductoDTO> product = new ComboBox<>();
        product.setPlaceholder("Producto");
        ...
        HorizontalLayout layout = new HorizontalLayout(product, quantity, price, remove);
        layout.setWidthFull();
        layout.expand(product);
        SaleItemRow row = new SaleItemRow(product, quantity, price, layout);
        rows.add(row);
        ...
        quantity.addValueChangeListener(event -> updateTotal.run());
        price.addValueChangeListener(event -> updateTotal.run());
    ...
    private record SaleItemRow(ComboBox<ProductoDTO> product, BigDecimalField quantity, BigDecimalField price, HorizontalLayout layout) {
        ...
        BigDecimal subtotal() {
            ...
            BigDecimal unit = value.cantidadMinimaPromo() != null && value.precioPromocional() != null
                    && quantity.getValue().compareTo(BigDecimal.valueOf(value.cantidadMinimaPromo())) >= 0
                    ? value.precioPromocional() : value.precioVenta();
            return unit.multiply(quantity.getValue());
        }

        private boolean canUse(BigDecimalField field) {
            return !field.isReadOnly() && field.getValue() != null;
        }
    }
}
