...
public class ComprasView extends VerticalLayout {
    ...
    public ComprasView(CompraService service,ProveedorService proveedores,ProductoService productos){this.service=service;this.proveedores=proveedores;this.productos=productos;
        ...
        grid.addColumn(CompraDTO::fecha).setHeader("Fecha");grid.addColumn(c->c.proveedor().nombre()).setHeader("Proveedor");grid.addColumn(CompraDTO::numeroComprobante).setHeader("Comprobante");
    ...
    private void form(){Dialog d=new Dialog();d.setHeaderTitle("Registrar compra");d.setWidth("min(850px,95vw)");
        ComboBox<ProveedorDTO> supplier=new ComboBox<>("Proveedor");supplier.setItems(proveedores.listar().stream().filter(ProveedorDTO::activo).toList());supplier.setItemLabelGenerator(ProveedorDTO::nombre);supplier.setWidthFull();
        TextField receipt=new TextField("N° comprobante");VerticalLayout rowsLayout=new VerticalLayout();List<Row> rows=new ArrayList<>();Button addItem=new Button("Agregar producto",e->row(rowsLayout,rows));
        d.add(new VerticalLayout(supplier,receipt,addItem,rowsLayout));row(rowsLayout,rows);Button save=new Button("Guardar y reponer stock",e->{try{
            if(supplier.getValue()==null||rows.stream().anyMatch(r->r.product.getValue()==null||r.quantity.getValue()==null||r.cost.getValue()==null))throw new IllegalArgumentException("Completá proveedor y productos");
            service.crear(new CompraCreateDTO(supplier.getValue().id(),receipt.getValue(),UserSession.getUser().id(),rows.stream().map(r->new ItemCompraCreateDTO(r.product.getValue().id(),r.baseQuantity(),r.cost.getValue(),r.expiration.getValue(),r.lot.getValue())).toList()));d.close();refresh();ViewSupport.success("Compra registrada y stock actualizado");
    ...
    private void row(VerticalLayout layout,List<Row> rows){ComboBox<ProductoDTO> product=new ComboBox<>();product.setItems(productos.listar());product.setItemLabelGenerator(ProductoDTO::nombre);product.setPlaceholder("Producto");product.setWidthFull();
        ...
        HorizontalLayout line=new HorizontalLayout(product,quantity,cost,lot,expiration,remove);line.setWidthFull();line.expand(product);Row row=new Row(product,quantity,cost,lot,expiration,line);rows.add(row);layout.add(line);product.addValueChangeListener(e->{if(e.getValue()!=null)cost.setValue(e.getValue().costo());});remove.addClickListener(e->{rows.remove(row);layout.remove(line);});}
